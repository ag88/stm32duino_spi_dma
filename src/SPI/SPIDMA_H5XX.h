...
#ifndef SPI_SPIDMAH5XX_H_
#define SPI_SPIDMAH5XX_H_

#if defined(STM32H5xx)
#include "SPIDMA.h"

class SPI_DMAH5 : public SPI_DMA {
public:
	SPI_DMAH5(uint32_t mosi = MOSI, uint32_t miso = MISO, uint32_t sclk = SCK, uint32_t ssel = PNUM_NOT_DEFINED)
...
typedef class SPI_DMAH5 SPI_DMAH5_SPI1;

/* SPI2 */
class SPI_DMAH5_SPI2 : public SPI_DMAH5 {
public:
	SPI_DMAH5_SPI2(uint32_t mosi = PB15, uint32_t miso = PB14, uint32_t sclk = PB13, uint32_t ssel = PNUM_NOT_DEFINED)
	  : SPI_DMAH5(mosi, miso, sclk, ssel) {};

	// in base class SPI_DMA4
	// virtual void begin();

	virtual ~SPI_DMAH5_SPI2();
...
/* SPI3 */
class SPI_DMAH5_SPI3 : public SPI_DMAH5 {
public:
	SPI_DMAH5_SPI3(uint32_t mosi = PB5, uint32_t miso = PB4, uint32_t sclk = PB3, uint32_t ssel = PNUM_NOT_DEFINED)
	  : SPI_DMAH5(mosi, miso, sclk, ssel) {};

	// in base class SPI_DMA4
	// virtual void begin();

	virtual ~SPI_DMAH5_SPI3();
